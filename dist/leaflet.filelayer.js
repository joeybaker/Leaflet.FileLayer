...
, FileLoader = L.Class.extend({
  ...
  , _loadGeoJSON: function(content){
    ...
    // if our filtering removed all the layers, error
    if (!geojson || objectIsEmpty(geojson._layers)){
      this.fire('data:error', {
        error: new Error('Parsed GeoJSON has no valid layers.')
        , message: 'No valid information in your upload.'
      })
      return
    }
